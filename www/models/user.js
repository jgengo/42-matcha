    ...
    static create(content) {
        return new Promise( (resolve, reject) => {
            bcrypt.hash(content.password, saltRounds, (err, hash) => {
                if (err) reject(err);
                connection.query('INSERT INTO users (first_name, last_name, email, password, created_at) VALUES (?, ?, ?, ?, NOW());', [content.firstName, content.lastName.toUpperCase(), content.email, hash],
                    (err, result) => {
